...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the moon python virtual environment."
  echo "Execute '. ./activate' before running."
  exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Moon Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in moon-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "Moon blockchain install-gui.sh completed."
echo ""
